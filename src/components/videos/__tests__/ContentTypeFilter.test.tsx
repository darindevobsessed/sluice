...
describe('ContentTypeFilter (Knowledge Bank)', () => {
  ...
  it('renders with "Videos" trigger text when selected is "youtube"', () => {
    const onChange = vi.fn()
    render(<ContentTypeFilter selected="youtube" onChange={onChange} />)

    expect(screen.getByText('Videos')).toBeInTheDocument()
  })

  it('renders with "Transcripts" trigger text when selected is "transcript"', () => {
    const onChange = vi.fn()
    render(<ContentTypeFilter selected="transcript" onChange={onChange} />)
  ...
  it('calls onChange with "youtube" when "Videos" is selected', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    render(<ContentTypeFilter selected="all" onChange={onChange} />)
  ...
  it('calls onChange with "transcript" when "Transcripts" is selected', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    render(<ContentTypeFilter selected="all" onChange={onChange} />)
